...
public class Admin {
	// 필드
	static PrintFormat pf = new PrintFormat();
...
//					//e.printStackTrace(); // 프로그램이 완료된 후에 반드시 제거 또는 주석
	...
	public static void customerChange()  {
		...
		try {
			...
			while (true) {
				System.out.print("주문내역을 볼 회원의 아이디를 입력하세요. (종료는 D를 입력하세요.)\n>>> ");
	...
	public static void customerAllSelect() {
		...
		try {
			...
			System.out.println("==================================================");
			System.out.format("+------------+-----------------+-----------------+-----------------+--------+------------+\n");
			System.out.printf("| %-8s | %-12s | %-11s | %-11s | %4s | %6s |\n", "이름", "아이디", "비밀번호", "전화번호", "쿠폰", "스탬프");
			System.out.format("+------------+-----------------+-----------------+-----------------+--------+------------+\n");
			...
			while (jdbc.RS.next()) {
				...
				String format = "| %-" + pf.convertByte(10, name) + "s | %-15s | %-15s | %-15s | %6s | %10s |\n";
				System.out.printf(format, name, id, pwd, phone, coupon, couponcheck);
			}
			System.out.format("+------------+-----------------+-----------------+-----------------+--------+------------+\n");
		} catch (ClassNotFoundException e) { // getConnection(url, user, password);
			//e.printStackTrace(); // 프로그램이 완료된 후에 반드시 제거 또는 주석
	...
	public static void selectOrderCart(String id) { // 날짜선택하면 불러오게 하는거 추후 변경***********************221106
		...
		try {
			...
			// 결과 출력
			System.out.println("\n==================================================");
			System.out.printf("* [%s]님의 주문 내역\n", id);
			System.out.println("==================================================");
			
			// 주문, 커피, 음료, 디저트, 이름, 수량, 가격, 날짜(달,요일)
			System.out.format("+--------+---------------------------+--------+--------+------------+\n");
			System.out.printf("| %-4s | %-23s | %-4s | %-4s | %-8s |\n", "주문", "이름", "수량", "가격", "날짜");
			System.out.format("+--------+---------------------------+--------+--------+------------+\n");			
			...
			while (jdbc.RS.next()) {
				...
				// 출력
				String format = "| %-6s | %-" + pf.convertByte(25, name) + "s | %6s | %6s | %-10s |\n";
				System.out.printf(format, ocode, name, amount, price, date);
			}
			System.out.format("+--------+---------------------------+--------+--------+------------+\n");	
	...
	public static void income() { // 날짜선택하면 불러오게 하는거 추후 변경***********************221106
		...
		try {
			...
			// 주문, 커피, 음료, 디저트, 이름, 수량, 가격, 날짜(달,요일)
			System.out.format("+------------+--------+---------------------------+--------+--------+------------+\n");
			System.out.printf("| %-7s | %-4s | %-23s | %-4s | %-4s | %-8s |\n","아이디","주문", "이름", "수량", "가격", "날짜");
			System.out.format("+------------+--------+---------------------------+--------+--------+------------+\n");
			...
			while (jdbc.RS.next()) {
				...
				// 출력
				String format = "| %-10s | %-6s | %-" + pf.convertByte(25, name) + "s | %6s | %6s | %-10s |\n";
				System.out.printf(format, id, ocode, name, amount, price, strNowDate);
